...
const toogleModalAuth = () => {
  if (modalAuth.classList.toggle('is-open')) {
    disableScroll();
  } else {
    enableScroll();
  };
...
checkAuth();
// Без перехода на другую стрицу
function createCardRestaurants(restaurant) {

  const { image, kitchen, name, price, products, stars, time_of_delivery: timeOfDelivery } = restaurant;// деструктуризация 
  const cardRestaurant = document.createElement('a');
  cardRestaurant.className = 'card crad-restaurant';
...
function renderCart() {
  ...
  // Подсчёт итогоавой суммы (reduce!!!)
...
function init() {
  ...
  inputSearch.addEventListener('keypress', (e) => {
    ...
    if(e.charCode === 13) {
      ...
      if (!value) { 
        e.target.value.style.backgroundColor = '#ff0000';
        e.target.value = '';
        setTimeout(() => {
      ...
      getData('../db/partners.json')
        ...
        .then(linksProduct => {
          cardsMenu.textContent = '';// очищаем от всех товаров
